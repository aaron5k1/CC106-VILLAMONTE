...
export default function Layout({ children, userProfile }: LayoutProps) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex font-sans text-[#1E293B]">
      ...
      <aside 
        className={cn(
          "bg-white border-r border-[#E2E8F0] transition-all duration-300 flex flex-col fixed inset-y-0 left-0 z-50 md:sticky h-screen",
          isSidebarOpen ? "w-64 translate-x-0" : "w-16 -translate-x-full md:translate-x-0",
        )}
      >
        <div className={cn(
          "p-6 mb-8 mt-2 flex items-center justify-between",
          !isSidebarOpen && "md:justify-center px-0 flex-col gap-2"
        )}>
          ...
          <button 
            onClick={() => setIsSidebarOpen(false)}
            className="p-2 text-[#64748B] hover:bg-slate-50 rounded-lg md:hidden"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {mainNavItems.map((item) => (
            ...
            >
              <item.icon className={cn(
                "w-5 h-5 shrink-0",
                isActive(item.path) ? "text-[#2563EB]" : "text-[#94A3B8] group-hover:text-[#1E293B]"
              )} />
        ...
        <div className="px-4 pb-8 space-y-4">
          <Link 
            to="/catalog"
            className={cn(
              "w-full bg-[#0F172A] text-white flex items-center justify-center gap-2 py-3 px-4 rounded-lg text-sm font-bold shadow-md hover:bg-[#1E293B] transition-colors",
          ...
          <div className="space-y-1">
            {bottomNavItems.map((item) => (
              <Link
                ...
                to={item.path}
                className={cn(
                  "flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold text-[#64748B] hover:bg-slate-50 hover:text-[#1E293B] transition-all group",
                  !isSidebarOpen && "md:justify-center md:px-0"
                )}
                title={!isSidebarOpen ? item.name : ''}
              >
            ...
            <button
              ...
              className={cn(
                "w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold text-[#64748B] hover:bg-red-50 hover:text-red-600 transition-all group",
                !isSidebarOpen && "md:justify-center md:px-0"
              )}
            >
        ...
        <header className="h-20 bg-[#F8FAFC]/80 backdrop-blur-md sticky top-0 z-40 flex items-center justify-between px-4 sm:px-8 border-b border-[#E2E8F0]">
          <div className="flex items-center gap-2 sm:gap-4 overflow-hidden">
            <button 
              ...
              className="p-2 text-[#64748B] hover:bg-slate-100 rounded-lg transition-colors shrink-0"
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <button 
                ...
                className={cn(
                  "p-2 text-[#64748B] hover:bg-slate-100 rounded-full transition-colors group relative",
                  notificationsOpen && "bg-slate-100 text-[#1E293B]"
                )}
              >
                ...
                {notificationsOpen && (
                  <>
                    ...
                    <motion.div
                      ...
                      className="absolute -right-28 sm:right-0 mt-4 w-[calc(100vw-2rem)] sm:w-96 max-w-[380px] bg-white border border-slate-200 rounded-3xl shadow-2xl shadow-slate-200/50 z-50 overflow-hidden"
                    >
                          ...
                          <div className="divide-y divide-slate-50">
                            {notifications.map((n) => (
                              <button
                                ...
                                className={cn(
                                  "w-full text-left p-4 sm:p-6 hover:bg-slate-50 transition-colors flex gap-3 sm:gap-4",
                                  !n.read && "bg-blue-50/30"
                                )}
                              >
                                <div className={cn(
                                  "w-2 h-2 mt-2 rounded-full flex-shrink-0",
                                  n.read ? "bg-slate-200" : "bg-[#2563EB]"
                                )} />
                                <div className="space-y-1">
            ...
            <Link to="/profile" className="flex items-center gap-3 pl-4 border-l border-[#E2E8F0] hover:opacity-80 transition-opacity">
              ...
              <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white shadow-sm ring-1 ring-slate-200">
                {userProfile?.photoURL ? (
                  ...
                  <div className="w-full h-full bg-[#EFF6FF] flex items-center justify-center text-[#2563EB] font-bold">
