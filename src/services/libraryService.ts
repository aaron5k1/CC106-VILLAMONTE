...
import { Book, Loan, Reservation, UserProfile, UserRole } from '../types';
...
export const libraryService = {
  ...
  async upsertBook(book: Partial<Book>): Promise<string> {
    try {
      if (book.id) {
  ...
  // Loans
  async borrowBook(userId: string, book: Book): Promise<void> {
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        if (!bookDoc.exists()) throw new Error('Book not found in archive');
        ...
        if (loanDoc.exists() && loanDoc.data()?.status === 'active') {
          throw new Error('You already have an active loan for this volume');
        }
  ...
  async returnBook(loan: Loan): Promise<void> {
  ...
  async updateReadingProgress(loanId: string, progress: number): Promise<void> {
    try {
  ...
  // Reservations
  async reserveBook(userId: string, book: Book): Promise<void> {
    try {
      ...
      if (!existing.empty) return; // Idempotent: already reserved
  ...
  async cancelReservation(resId: string): Promise<void> {
    try {
  ...
  async ensureUserProfile(user: any): Promise<UserProfile> {
    try {
      ...
      const role: UserRole = user.email === 'aaronjamesvillamonte@gmail.com' ? 'admin' : 'member';
  ...
  async seedBooks(): Promise<void> {
    ...
    for (const b of books) {
      batch.push(addDoc(collection(db, BOOKS_COL), {
        quantity: 5,
        availableCount: 5,
        ...b,
        createdAt: serverTimestamp()
