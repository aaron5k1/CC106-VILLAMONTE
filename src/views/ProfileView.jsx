...
export default function ProfileView({ userProfile }) {
    ...
    return (<div className="max-w-4xl mx-auto space-y-8 pb-12">
      {/* Edit Overlay */}
      <AnimatePresence>
        {isEditing && (<div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsEditing(false)} className="absolute inset-0 bg-[#0F172A]/40 backdrop-blur-sm"/>
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} className="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-10">
              ...
              <form onSubmit={handleSave} className="space-y-6">
                ...
                <div className="flex gap-4">
                  ...
                  <button type="button" onClick={() => setIsEditing(false)} className="px-6 py-4 bg-slate-100 text-slate-600 rounded-xl font-bold hover:bg-slate-200 transition-all">
                    Cancel
                  </button>
                </div>
              </form>
      ...
      <div className="px-4 sm:px-8 -mt-16 sm:-mt-20 relative z-10">
        <div className="flex flex-col md:flex-row items-center md:items-end gap-6 mb-8 text-center md:text-left">
          <div className="relative group">
            <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-3xl border-4 border-white bg-slate-100 overflow-hidden shadow-xl shadow-slate-200 ring-1 ring-slate-100">
              {userProfile.photoURL ? (<img src={userProfile.photoURL} alt="Profile" className="w-full h-full object-cover" referrerPolicy="no-referrer"/>) : (<div className="w-full h-full flex items-center justify-center text-[#2563EB] text-4xl sm:text-5xl font-bold bg-[#EFF6FF]">
          ...
          <div className="flex-1 pb-0 sm:pb-4">
            <div className="flex flex-col md:flex-row items-center gap-3 mb-1">
              ...
              {userProfile.role === 'admin' && (<span className="bg-blue-100 text-[#2563EB] text-[10px] font-bold uppercase tracking-widest px-2 py-0.5 rounded flex items-center gap-1 w-fit">
          ...
          <div className="pb-0 sm:pb-4">
            <button onClick={() => setIsEditing(true)} className="flex items-center gap-2 px-6 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-bold text-slate-700 hover:bg-slate-50 transition-all shadow-sm">
        ...
        ].map((badge) => (<div key={badge.label} className="flex items-center gap-4 group">
