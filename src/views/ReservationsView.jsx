...
import { Calendar, Clock, Trash2, BookOpen, CheckCircle2, AlertCircle, ArrowRight } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
export default function ReservationsView({ userProfile }) {
    ...
    return (<div className="max-w-[1400px] mx-auto space-y-12 pb-24">
      {/* Header Section */}
      ...
      <section className="space-y-6">
        ...
        {pendingReservations.length === 0 ? (<div className="bg-[#F8FAFC] border-2 border-dashed border-slate-200 rounded-3xl p-16 text-center">
            ...
            <Link to="/catalog" className="mt-8 bg-[#0F172A] text-white px-8 py-4 rounded-md font-bold text-sm hover:bg-[#1E293B] transition-all shadow-xl shadow-slate-200 inline-flex items-center gap-2">
              Explore Catalog <ArrowRight className="w-4 h-4"/>
            </Link>
          </div>) : (<div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
            <AnimatePresence mode="popLayout">
              {pendingReservations.map((res) => (<motion.div key={res.id} layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl border border-slate-200 p-6 flex gap-6 shadow-sm group hover:shadow-md transition-all relative overflow-hidden">
                  <div className="w-32 h-44 bg-slate-100 rounded-lg overflow-hidden shadow-lg shrink-0">
                    <img src={res.bookCoverUrl || `https://picsum.photos/seed/${res.bookId}/400/600`} alt={res.bookTitle} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"/>
                  </div>
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div className="space-y-2">
                      <div className="flex justify-between items-start">
                        <span className={cn("px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border flex items-center gap-1.5", getStatusColor(res.status))}>
                          {getStatusIcon(res.status)}
                          {res.status}
                        </span>
                        <button onClick={() => handleDelete(res.id)} className="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Remove Reservation">
      ...
      {/* History Section */}
      {pastReservations.length > 0 && (<section className="space-y-8 pt-12 border-t border-slate-100">
          <div className="flex items-center justify-between">
            ...
            <div className="overflow-x-auto">
              <table className="w-full border-collapse">
                ...
                <tbody className="divide-y divide-slate-50">
                  {pastReservations.map((res) => (<tr key={res.id} className="group hover:bg-slate-50/50 transition-colors">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-14 bg-slate-100 rounded overflow-hidden shrink-0 shadow-sm">
                            <img src={res.bookCoverUrl || `https://picsum.photos/seed/${res.bookId}/400/600`} alt={res.bookTitle} className="w-full h-full object-cover"/>
                          </div>
                          <div>
                      ...
                      <td className="px-8 py-6">
                        <span className={cn("px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border inline-flex items-center gap-1.5", getStatusColor(res.status))}>
                          {getStatusIcon(res.status)}
                          {res.status}
                        </span>
                      </td>
                      <td className="px-8 py-6 text-right">
                        <button onClick={() => handleDelete(res.id)} className="p-2 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all" title="Purge Record">
                          <Trash2 className="w-4 h-4"/>
                        </button>
                      </td>
                    </tr>))}
