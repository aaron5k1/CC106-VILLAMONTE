...
import { AlertTriangle, Clock, History, BookOpen, ArrowRight, RotateCcw, CheckCircle2 } from 'lucide-react';
...
const CountdownTimer = ({ targetDate }) => {
    ...
    const mins = Math.max(0, differenceInMinutes(targetDate, now) % 60);
    if (days > 0)
        return <span>{days}d {hours}h left</span>;
    if (hours > 0)
        return <span>{hours}h {mins}m left</span>;
    return <span>{mins}m left</span>;
};
export default function MyBooksView({ userProfile }) {
    ...
    const activeLoans = loans.filter(l => l.status === 'active' && (l.readingProgress || 0) < 100);
    const pastLoans = loans.filter(l => l.status === 'returned' || (l.readingProgress !== undefined && l.readingProgress >= 100));
    ...
    return (<div className="max-w-[1400px] mx-auto space-y-12 pb-12">
      ...
      {overdueLoans.length > 0 && (<section className="space-y-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
            {overdueLoans.map((loan) => (<motion.div key={loan.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-xl border border-slate-200 p-6 sm:p-8 flex flex-col sm:flex-row gap-6 sm:gap-8 shadow-sm hover:shadow-md transition-all group">
                <div className="w-full sm:w-48 h-64 sm:h-64 flex-shrink-0 bg-slate-100 rounded-lg overflow-hidden shadow-lg group-hover:scale-[1.02] transition-transform">
                  <img src={loan.bookCoverUrl || `https://picsum.photos/seed/${loan.bookId}/400/600`} alt={loan.bookTitle} className="w-full h-full object-cover" referrerPolicy="no-referrer"/>
                </div>
                <div className="flex flex-col flex-1">
                  ...
                  <div className="mt-auto space-y-4">
                    ...
                    <button onClick={() => handleReturn(loan)} disabled={returningId === loan.id} className="w-full bg-red-600 hover:bg-red-700 text-white py-4 rounded-lg font-bold transition-all shadow-lg shadow-red-100 flex items-center justify-center gap-2 disabled:opacity-50">
                      {returningId === loan.id ? (<div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"/>) : (<>
      ...
      <section className="space-y-6">
        ...
        {currentLoans.length === 0 ? (<div className="py-20 border-2 border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center bg-white text-center px-6">
            ...
            {currentLoans.map((loan) => {
                ...
                return (<motion.div key={loan.id} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-all group relative">
                  <div className="aspect-[3/4] relative overflow-hidden">
                    <img src={loan.bookCoverUrl || `https://picsum.photos/seed/${loan.bookId}/400/600`} alt={loan.bookTitle} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" referrerPolicy="no-referrer"/>
                    <div className="absolute top-4 right-4 text-center">
                      <span className="bg-white/90 backdrop-blur-md text-[#1E293B] text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded shadow-sm border border-slate-200/50">
                        <CountdownTimer targetDate={loan.dueDate.toDate()}/>
                      </span>
                    </div>
                  </div>
                  <div className="p-6 space-y-4">
                    ...
                    <div className="flex flex-col gap-2 pt-4 border-t border-slate-50">
                      <Link to={`/read/${loan.id}`} className="w-full bg-[#0F172A] text-white py-2.5 rounded-lg text-xs font-bold uppercase tracking-widest hover:bg-[#1E293B] transition-all flex items-center justify-center gap-2">
                      ...
                      <div className="flex items-center justify-between pt-2">
                        ...
                        <button onClick={() => handleReturn(loan)} disabled={returningId === loan.id} className="p-2 text-slate-400 hover:text-red-500 transition-colors" title="Return Book">
      ...
      <section ref={historyRef} className="space-y-8 pt-12 border-t border-slate-100">
        ...
        {pastLoans.length === 0 ? (<div className="py-20 bg-slate-50 rounded-3xl flex flex-col items-center justify-center text-center px-6">
              ...
              <table className="w-full text-left">
                <thead className="bg-slate-50 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-widest text-[#A39E93]">Status/Return</th>
                    <th className="px-8 py-5 text-[10px] font-bold uppercase tracking-widest text-[#A39E93]">Info</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {pastLoans.map((loan) => (<tr key={loan.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-8 py-6 text-sm text-slate-500">{loan.returnDate ? formatDate(loan.returnDate) : 'Retained (Completed)'}</td>
                      <td className="px-8 py-6">
                         <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-emerald-50 text-emerald-600 border border-emerald-100">
                           ...
                           {loan.status === 'returned' ? 'Returned' : 'Completed'}
                         </span>
                      </td>
                    </tr>))}
                </tbody>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pt-8">
          ...
          <div className="flex-1 space-y-6 relative z-10 w-full">
            ...
            <div className="pt-4 sm:pt-8 w-full">
              ...
              <div className="h-2.5 sm:h-3 bg-slate-200 rounded-full overflow-hidden">
                <motion.div initial={{ width: 0 }} animate={{ width: '84%' }} transition={{ duration: 1.5, ease: "easeOut" }} className="h-full bg-[#1E293B]"/>
              </div>
            </div>
          </div>
        ...
        <div className="bg-[#0F172A] rounded-3xl p-8 sm:p-10 flex flex-col justify-between text-white relative overflow-hidden group min-h-[250px]">
           ...
           <div className="relative z-10">
             <button onClick={scrollToHistory} className="flex items-center gap-2 font-bold hover:gap-4 transition-all">
