...
import { ChevronLeft, Bookmark, Moon, Sun, ChevronRight, Clock } from 'lucide-react';
import { motion } from 'framer-motion';
export default function ReaderView({ userProfile }) {
    ...
    const handleNextChapter = () => {
        ...
        if (nextProgress >= 100) {
            console.log('You have completed this volume. Knowledge verified.');
        }
    };
    if (loading) {
        return (<div className="fixed inset-0 bg-white z-[60] flex items-center justify-center">
    ...
    if (!book)
        return null;
    return (<div className={`fixed inset-0 z-[60] flex flex-col transition-colors duration-500 ${isDarkMode ? 'bg-[#0F172A] text-slate-300' : 'bg-[#FDFCFB] text-[#1E293B]'}`}>
      {/* Reader Header */}
      <header className={`h-16 px-6 flex items-center justify-between border-b ${isDarkMode ? 'border-white/10 bg-slate-900/50' : 'border-slate-100 bg-white/80'} backdrop-blur-md`}>
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/my-books')} className="p-2 hover:bg-black/5 rounded-lg transition-colors">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 hover:bg-black/5 rounded-lg transition-colors">
            {isDarkMode ? <Sun className="w-5 h-5"/> : <Moon className="w-5 h-5"/>}
          </button>
          <div className="h-4 w-px bg-slate-200 mx-2"/>
          <button onClick={() => setFontSize(f => Math.max(12, f - 2))} className="p-2 hover:bg-black/5 rounded-lg transition-colors text-xs font-bold">
            A-
          </button>
          <button onClick={() => setFontSize(f => Math.min(32, f + 2))} className="p-2 hover:bg-black/5 rounded-lg transition-colors text-lg font-bold">
            A+
          </button>
          <div className="h-4 w-px bg-slate-200 mx-2"/>
      ...
      <main className="flex-1 overflow-y-auto px-6 py-12">
        <article className="max-w-2xl mx-auto space-y-8">
          ...
          <div className="font-serif leading-relaxed space-y-6" style={{ fontSize: `${fontSize}px` }}>
            ...
            <img src={book.coverUrl} alt="Manuscript Detail" className="w-full h-[400px] object-cover rounded-3xl my-12 shadow-2xl opacity-80"/>
      ...
      <footer className={`h-20 px-6 flex items-center justify-between border-t ${isDarkMode ? 'border-white/10 bg-slate-900/50' : 'border-slate-100 bg-white/80'} backdrop-blur-md`}>
        <div className="flex flex-col gap-1 flex-1 max-w-[200px]">
          ...
          <div className="h-1 bg-slate-200 rounded-full overflow-hidden">
            <motion.div initial={{ width: 0 }} animate={{ width: `${progress}%` }} className="h-full bg-[#2563EB]"/>
          </div>
        </div>

        <div className="flex items-center gap-4">
          <button onClick={handleNextChapter} disabled={progress >= 100} className={`px-6 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all ${isDarkMode ? 'bg-white text-slate-900 group' : 'bg-[#0F172A] text-white hover:bg-[#1E293B] group'} disabled:opacity-50`}>
